...
class SearchTelemetryManager:
    ...
    def __init__(
        ...
        self.writer = SummaryWriter(log_dir=str(self.log_dir)) if HAS_TENSORBOARD else None
    ...
    def get_cuda_vram_stats() -> Dict[str, float]:
        ...
        if not torch.cuda.is_available():
            return {"vram_allocated_mb": 0.0, "vram_reserved_mb": 0.0, "vram_max_allocated_mb": 0.0}
        ...
        return {
            ...
            "vram_max_allocated_mb": round(torch.cuda.max_memory_allocated(0) / (1024 ** 2), 2),
        }
    ...
    def broadcast_event_sync(self, event_type: str, data: Dict[str, Any]):
        ...
        payload = json.dumps({"event": event_type, "timestamp": time.time(), "data": data})
        stale_clients = set()
    ...
    def log_generation(self, gen_metrics: Dict[str, Any]):
        ...
        if self.writer:
            self.writer.add_scalar("Fitness/Global_Best", gen_metrics.get("best_fitness", 0.0), gen)
            self.writer.add_scalar("Fitness/Mean", gen_metrics.get("mean_fitness", 0.0), gen)
            self.writer.add_scalar("Hardware/Best_Latency_ms", gen_metrics.get("best_latency_ms", 0.0), gen)
            self.writer.add_scalar("Hardware/Best_Params", gen_metrics.get("best_params", 0), gen)
            self.writer.add_scalar("VRAM/Allocated_MB", vram["vram_allocated_mb"], gen)
            self.writer.add_scalar("VRAM/Max_Allocated_MB", vram["vram_max_allocated_mb"], gen)
            self.writer.flush()
    ...
    def notify_pareto_discovery(self, candidate_id: str, accuracy: float, latency_ms: float, params: int):
        ...
        self.pareto_buffer.append({"candidate_id": candidate_id, "accuracy": accuracy, "latency_ms": latency_ms, "params": params})
